...
export function AuthPage({ mode }) {
  ...
  return (
    <main className="relative grid min-h-screen place-items-center overflow-hidden bg-app-bg p-3 text-app-text md:p-5">
      <motion.div
      ...
      <motion.section
        ...
        className="relative z-10 grid w-full max-w-5xl gap-4 overflow-hidden rounded-[28px] border border-white/10 bg-gradient-to-br from-[var(--color-hero-start)] via-[var(--color-hero-mid)] to-[var(--color-hero-end)] p-3 shadow-[0_35px_90px_rgba(0,0,0,0.35)] md:min-h-[min(860px,calc(100vh-40px))] md:grid-cols-2 md:p-4"
      >
        <motion.div
          initial={{ opacity: 0, x: -24 }}
          animate={{ opacity: 1, x: 0 }}
          transition={{ duration: 0.6, ease: "easeOut", delay: 0.08 }}
          className="order-2 grid content-between gap-7 rounded-[24px] border border-white/10 bg-white/[0.05] p-5 text-white backdrop-blur md:order-1 md:p-8"
        >
          <div>
            ...
            <h1 className="mt-5 max-w-xl text-4xl font-black leading-[1.06] tracking-normal text-white md:text-[44px] xl:text-5xl">
            ...
            {!isSignup && (
              <motion.section
                ...
                className="auth-glass mt-6 max-w-md p-4 text-blue-50"
              >
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p className="text-base font-bold text-white">Try Demo</p>
                    <p className="mt-1 text-sm leading-5 text-blue-100/75">Use the seeded account to explore the analyzer.</p>
                  </div>
                ...
                <div className="mt-4 grid gap-2 rounded-[18px] border border-white/10 bg-white/[0.06] p-3 text-sm">
                ...
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  <button type="button" className="button-quiet min-h-11 rounded-2xl border-white/10 bg-white/[0.07] hover:bg-white/[0.11]" onClick={copyDemoCredentials}>
                    {copied ? <Check size={16} /> : <Clipboard size={16} />}
                    {copied ? "Copied" : "Copy Credentials"}
                  </button>
                  <button type="button" className="button-primary group relative min-h-11 overflow-hidden rounded-2xl bg-gradient-to-r from-[var(--color-accent-indigo)] to-[var(--color-cta-end)] shadow-lg shadow-indigo-500/25 hover:scale-[1.01]" onClick={useDemoAccount}>
          ...
          <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-3">
            {["ATS score", "Skill gaps", "Rewrite cues"].map((item) => (
              <span key={item} className="rounded-2xl border border-white/10 bg-white/10 p-3 text-center font-bold text-blue-50 shadow-lg shadow-blue-950/10 transition duration-200 hover:-translate-y-1 hover:bg-white/[0.13]">{item}</span>
            ))}
          </div>
        </motion.div>
        <motion.form
          ...
          onSubmit={submit}
          className="auth-card order-1 mx-auto grid w-full max-w-[460px] content-center gap-4 self-center p-5 md:order-2 md:p-8 lg:p-9"
        >
          <div>
            <p className="eyebrow">{isSignup ? "Create account" : "Welcome back"}</p>
            <h2 className="mt-2 text-3xl font-black leading-tight tracking-normal text-white">{isSignup ? "Start analyzing" : "Sign in"}</h2>
          ...
          <button className="button-primary group relative min-h-14 overflow-hidden rounded-2xl bg-gradient-to-r from-[var(--color-accent-indigo)] to-[var(--color-cta-end)] text-base shadow-lg shadow-indigo-500/25 hover:scale-[1.01]" disabled={status === "loading"}>
            <span className="absolute inset-0 -translate-x-full bg-white/15 transition-transform duration-500 group-hover:translate-x-full" />
